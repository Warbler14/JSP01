...
public class CommandLoader022 implements CommandLoader{
	static Logger logger = Logger.getLogger(CommandLoader022.class);
	
	private String move_page [][] ;
	private String parameters [] ;
	
	public CommandLoader022( String [][] move_page, String [] parameters ){
	...
	private static int [][] rotateLeft( int [][] matrix ){
		...
		for( int iy = 0, ly = matrix.length ; iy<ly ; iy++ ){
			...
			for( int ix = 0, lx = matrix[0].length ; ix<lx ; ix++ ){
				...
				matrix2[ix][(matrix.length-1)-iy] = matrix[iy][ix];
