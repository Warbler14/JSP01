...
/**
 * http://localhost:8081/JSP01/drawDiamond?width=500&height=500&round=20
 */
@WebServlet("/drawDiamond")
public class ImageGet021 extends HttpServlet {
	private static final long serialVersionUID = 1L;
	
	static Logger logger = Logger.getLogger(ImageGet021.class);
	...
	private static void paintProcessJPG( HttpServletResponse response
		...
		for(int i = 0,  ii = round/2 ; i<ii  ; i++, colorIndex++ ){
			...
			drawArea = drawLines( drawArea, new int[] {dataArray[0]/2,dataArray[1]/2}, divX, divY, g );
		}
	...
	private static int [][] drawLines( int [][] drawArea, int[] mid, int divX, int divY, Graphics2D g ){
		/*int x1 = drawArea[0][0] + divX;
		int y1 = drawArea[0][1] + divY;
		int x2 = drawArea[1][0] - divX;
		int y2 = drawArea[1][1] - divY;*/
		
		int [][] dataArea = new int[][]{  {mid[0], drawArea[0][1]}
										, {drawArea[0][0], mid[1]}
										, {mid[0], drawArea[1][1]}
										, {drawArea[1][0], mid[1]}};
		...
		g.drawLine(dataArea[0][0], dataArea[0][1], dataArea[1][0], dataArea[1][1]);
		g.drawLine(dataArea[1][0], dataArea[1][1], dataArea[2][0], dataArea[2][1]);
		g.drawLine(dataArea[2][0], dataArea[2][1], dataArea[3][0], dataArea[3][1]);
		g.drawLine(dataArea[3][0], dataArea[3][1], dataArea[0][0], dataArea[0][1]);
		
		int [][] drawArea2 = new int[][]{  {drawArea[0][0] + divX, drawArea[0][1] + divY}
